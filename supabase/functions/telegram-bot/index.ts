...
const WEBAPP_BASE = "https://id-preview--13572394-3347-4564-a651-8996fe1cafa4.lovable.app/app";
...
// Per-invocation chat throttle (helps avoid Telegram 429)
const lastSentByChat = new Map<number, number>();

async function sendTg(botToken: string, payload: any, endpoint = "sendMessage"): Promise<any> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (res.status === 403) return json; // user blocked the bot — silent
    return json;
  }
}
...
Deno.serve(async (req) => {
  ...
  const BOT_TOKEN = Deno.env.get("TELEGRAM_BOT_TOKEN_V2") || Deno.env.get("TELEGRAM_BOT_TOKEN_V2") || Deno.env.get("TELEGRAM_BOT_TOKEN_NEW") || Deno.env.get("TELEGRAM_BOT_TOKEN")!;
...
async function handleUpdate(update: any, supabase: any, botToken: string, groupId: string, supabaseUrl: string) {
  ...
  if (message) {
    ...
    if (user.is_banned) return jsonResponse({ ok: true });
    ...
    if (user.captcha_pending) {
      ...
      if (!isNaN(userAnswer) && userAnswer === user.captcha_answer) {
        ...
        const parts = String(user.captcha_pending).split("+");
        ...
        if (newCount >= 3) {
          await supabase.from("admin_alerts").insert({
            type: "fraud",
            user_id: user.id,
            message: `❗️ Юзер @${user.username || user.telegram_id} провалил капчу 3 раза подряд`,
          });
        }
        await sendTg(botToken, {
          chat_id: chatId,
          text: `🔒 Решите пример: *${parts[0]} + ${parts[1]} = ?*\nОтправьте число.`,
          parse_mode: "Markdown",
        });
        return jsonResponse({ ok: true });
      }
    ...
    // ONLY respond to explicit commands. Replying to every text was the spam pattern
    // that got the bot rate-limited by Telegram.
  ...
  if (callback) {
    ...
    else if (cbData.startsWith("task_")) await handleTaskComplete(chatId, botToken, user, cbData.slice(5), supabase);
...
async function sendMainInline(chatId: number, botToken: string, user: any, _supabaseUrl: string) {
  const webAppUrl = `${WEBAPP_BASE}?user_id=${user.telegram_id}`;
...
async function handleTasks(chatId: number, botToken: string, user: any, supabase: any) {
  ...
  await sendTg(botToken, {
    chat_id: chatId,
    text: `📋 Доступно заданий: *${available.length}*`,
    parse_mode: "Markdown",
...
async function handleWithdraw(chatId: number, botToken: string, user: any, supabase: any) {
  if (user.balance_frozen) {
    await sendTg(botToken, { chat_id: chatId, text: "🧊 Баланс заморожен." });
    return;
  }
  ...
  if (balance < minWithdraw) {
    await sendTg(botToken, {
      chat_id: chatId,
      text: `⭐️ Минимум: ${minWithdraw} PT\n💎 У вас: *${balance.toFixed(1)} PT*`,
      parse_mode: "Markdown",
    });
    return;
  }
  ...
  const buttons: any[][] = [];
  if (balance >= 100) buttons.push([{ text: `100 PT → ${Math.floor(100 / rate)}⭐️`, callback_data: "withdraw_100" }]);
  if (balance >= 500) buttons.push([{ text: `500 PT → ${Math.floor(500 / rate)}⭐️`, callback_data: "withdraw_500" }]);
  if (balance >= 1000) buttons.push([{ text: `1000 PT → ${Math.floor(1000 / rate)}⭐️`, callback_data: "withdraw_1000" }]);
...
async function processWithdraw(chatId: number, botToken: string, user: any, amountPt: number, supabase: any, groupId: string) {
  ...
  if (Number(user.balance_pt) < amountPt) {
    await sendTg(botToken, { chat_id: chatId, text: "❌ Недостаточно средств." });
    return;
  }
  const { data: settings } = await supabase.from("settings").select("value").eq("key", "exchange_rate").single();
  const rate = parseFloat(settings?.value || "1");
  ...
  const { data: ips } = await supabase
    .from("user_ips").select("ip_address").eq("user_id", user.id)
  ...
  await sendTg(botToken, {
    chat_id: chatId,
    text: `✅ Заявка: ${amountPt} PT → ⭐️ ${amountStars} принята.`,
  });

  const report =
    `📤 *Заявка на вывод*\n` +
    `👤 @${user.username || user.telegram_id}\n` +
    `💎 ${amountPt} PT → ⭐️ ${amountStars}\n` +
    `🌐 ${userIp}\n` +
...
async function handleTaskComplete(chatId: number, botToken: string, user: any, taskId: string, supabase: any) {
  ...
  if (!task) {
    await sendTg(botToken, { chat_id: chatId, text: "❌ Задание не найдено." });
    return;
  }
  ...
  const { data: existing } = await supabase.from("task_completions")
    .select("id").eq("user_id", user.id).eq("task_id", taskId).maybeSingle();
  if (existing) {
    await sendTg(botToken, { chat_id: chatId, text: "✅ Уже выполнено." });
    return;
  }
  ...
  if (task.type === "subscribe" && task.channel_id) {
    ...
    if (!isMember) {
      const link = task.channel_username ? `https://t.me/${String(task.channel_username).replace("@", "")}` : "";
      await sendTg(botToken, {
        chat_id: chatId,
        text: `❌ Сначала подпишитесь${link ? `: ${link}` : ""}`,
      });
      return;
    }
  }
