...
// ПРОВЕРКА SECRET_TOKEN УДАЛЕНА РУКАМИ ДЛЯ ИСПРАВЛЕНИЯ ОШИБКИ 401
return await handleUpdate(body, supabase, BOT_TOKEN, GROUP_ID, SUPABASE_URL);
}

if (body.action === "set_webhook") {
const webhookUrl = ${SUPABASE_URL}/functions/v1/telegram-bot; const res = await fetch(``${TELEGRAM_API}${BOT_TOKEN}/setWebhook`, {
method: "POST",
...
await sendMainInline(chatId, botToken, { ...user, 

captcha_pending: null }, supabaseUrl);
return jsonResponse({ ok: true });
} else {
...
type: "fraud",
user_id: user.id,
message: ❗️ Юзер @${user.username || user.telegram_id} провалил капчу 
 

{user.username || user.telegram_id} — провал капчи 
...
else if (cbData.startsWith("task_")) await handleTaskComplete(chatId, botToken, user, 

cbData.slice(5), supabase, botToken);
...
const webAppUrl = https://id-preview--13572394-3347-4564-a651-8996fe1cafa4.lovable.app/app?user_id=${user.telegram_id}; const text = vary(💎 Баланс: *${Number(user.balance_pt).toFixed(1)} PT*\n\nВыберите действие:);
const keyboard = [
...
[{ text: "⭐️ Вывод", callback_data: "menu_withdraw" }],
];
await sendTg(botToken, { chat_id: chatId, text, parse_mode: "Markdown", reply_markup: { inline_keyboard: keyboard } });
}

async function handleDailyBonus(chatId: number, botToken: string, user: any, supabase: 
...
await sendTg(botToken, { chat_id: chatId, text: ⏰ Уже получено. Следующий через *${h} ч.*, parse_mode: "Markdown" }); return; } } const bonus = Math.round((1.5 + Math.random() * 1.5) * 10) / 10; const newBalance = Number(user.balance_pt) + bonus; await supabase.from("users").update({ balance_pt: newBalance, daily_bonus_at: now.toISOString() }).eq("id", user.id); await sendTg(botToken, { chat_id: chatId, text:🎁 Бонус: +{bonus} PT*\n💎 Баланс: *{newBalance.toFixed(1)} PT`, parse_mode: "Markdown" });
...
let label = "";
if (t.type === "subscribe") label = 📢 Подписка — ${t.reward_pt} PT; else if (t.type === "view_post") label =👁 Просмотр — ${t.reward_pt} PT;
else if (t.type === "reaction") label = ❤️ Реакция — ${t.reward_pt} PT; if (label) buttons.push([{ text: label, callback_data:task_${t.id} }]);
});
...
await sendTg(botToken, { chat_id: chatId, text: ⭐️ Минимум: ${minWithdraw} PT\n💎 У вас: *$

{balance.toFixed(1)} PT*, parse_mode: "Markdown" });
return;
}
const stars = Math.floor(balance / rate);
const text = ⭐️ *Вывод*\n💎 ${balance.toFixed(1)} PT → ⭐️ ${stars}\nКурс: 1⭐️ = ${rate} PT; const buttons: any[][] = []; if (balance &gt;= 100) buttons.push([{ text:100 PT → ${Math.floor(100/rate)}⭐️, callback_data: "withdraw_100" }]);
if (balance >= 500) buttons.push([{ text: 500 PT → ${Math.floor(500/rate)}⭐️, callback_data: "withdraw_500" }]); if (balance &gt;= 1000) buttons.push([{ text:1000 PT → ${Math.floor(1000/rate)}⭐️, callback_data: "withdraw_1000" }]);
buttons.push([{ text: Всё (${balance.toFixed(0)} → ${stars}⭐️), callback_data: withdraw_${Math.floor(balance)}` }]);
...
const { data: ips } = await 

supabase.from("user_ips").select("ip_address").eq("user_id", user.id).order("last_seen_at", { ascending: false }).limit(1);
...
await sendTg(botToken, { chat_id: chatId, text: ✅ Заявка:${amountPt} PT → 

 
{user.username || user.telegram_id}, ${amountPt} PT,
});
}
...
{task.channel_username.replace("@", "")}: ""; await sendTg(botToken, { chat_id: chatId, text:❌ Сначала подпишитесь
...
user_id: user.id, task_id: taskId, check_at: checkAt });
}
